                ...
                if (s >= 0) {
                    if (connect(s, res->ai_addr, res->ai_addrlen) == 0) {
                        ...
                        if (action_http->method_n == 3) {
                            ...
                            action_http->len = 70 + strlen(method) + ((method_req != NULL) ? strlen(method_req) : 0) + strlen(FIRMWARE_VERSION) + strlen(action_http->host) +  strlen(action_http->url) + content_len_n;
...
void user_init(void) {
    ...
    if (status == SYSPARAM_OK) {
        printf("ESPY House sysparam ready");
    } else {
        printf("! ESPY House sysparam. Creating...");
    ...
    snprintf(main_config.name_value, 12, "ESPY-%02X%02X%02X", macaddr[3], macaddr[4], macaddr[5]);
